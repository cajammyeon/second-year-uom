...
class Breakpoint {
  public:
     ...
     */
    Breakpoint(addr_t address, std::string name);
...
class Emulator {
  public:
     ...
     */
    int insert_breakpoint(addr_t address, std::string name);
     ...
     */
    const Breakpoint* find_breakpoint(std::string name) const;
     ...
     * @return Whether a breakpoint was removed (1 means removed, 0 means none removed)
     */
    int delete_breakpoint(std::string name);
    ...
    // default destructor, for clang-tidy
    ~Emulator() = default;
