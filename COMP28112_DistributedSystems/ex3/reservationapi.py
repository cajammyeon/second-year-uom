...
class ReservationApi:
    ...
    def _send_request(self, method: str, endpoint: str) -> dict:
        ...
        while (number_of_tries <= self.retries) :

            response = method_dict[method](endpoint, headers = self._headers())
            ...
            if (response.status_code == 200) :
                ...
                print(f"Error reason : {self._reason(response)}. \nCode {response.status_code}. Retrying ({number_of_tries}) ...")
            else :
                raise exception_dict.get(response.status_code, exit)
