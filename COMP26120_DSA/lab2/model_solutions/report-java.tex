...
set label 1 at 15000,0.34 mq_value
...
\label{fig:sorted1}
\end{figure}
\newcommand{\sortedoneq}{0.25}
\newcommand{\sortedonem}{0.000002}
As can be seen from the graph the the line $f(x) = mx + q$ has a reasonable fit\footnote{As is often the case with Java, there is a wider range of timings thatnwe see for C or Python (take a look at the corresponding reports to see) which can make results harder to interpret.  I wanted to perform the experiment identically for all three languages, but in a real situation, with these results, I'd probably use more than five dictionaries, and run the program on each dictionary several times and then plot the graph over the average of the timings to see if I could get a clearer picture of what is happening.} and the values of \sortedonem\ and \sortedoneq\ have been computed for $m$ and $q$ respectively\footnote{For this exercise it is fine to judge fit by eye, but in a scientific experiment, particularly if there is a lot of noise in the data you should use statistical methods to evaluated how good a fit is.}.  This confirms the hypothesis about the behaviour of the algorithm on sorted input.  It allows us to predict the time taken to sort a dictionary of size $x$ as $\sortedonem x + \sortedoneq$.  %However the value of 0.005 is probably incorrect.  It would suggest that for small dictionaries the algorithm would execute in negative time.  This is probably the result of the set up time being negligible and the noise in the time system causing a miscalculation.  Further experimentation with even larger dictionaries would help remove this error.
...
\newcommand{\sortedtwom}{0.00047 \times 10^{-5}}
...
\newcommand{\crossover}{1.75}
\subsection{Discussion}
As can be seen from the graphs the lines for $f1(x)$ and $f2(x)$ cross at around 1 for smaller query files and around \crossover\ for larger query files\footnote{Again there is more variation here than in C and Python with the crossover point ranging from 1.5 up to just below 2.} .  This confirms our hypothesis that it becomes quicker to sort the dictionary of size $n$ using insertion sort and then use binary search to check the query, as opposed to using linear search on the unsorted dictionary somewhere between $\frac{n}{10}$ queries and $10n$ queries.  In fact it tells us that this point occurs somewhere between 0.5$n$ and $n$ queries.
