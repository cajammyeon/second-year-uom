...
int total_comparisons(struct bstree* tree){
  if(tree){
    return tree->comparisons + total_comparisons(tree->left) + total_comparisons(tree->right);
  }
  return 0;
}
...
{
    printf("Height: %d\n", height(tree));
    printf("Comparisons per insert: %d\n", total_comparisons(tree)/size(tree));
