...
if len(sys.argv) < 4:
  print("Run as test.py <language> <problems> <structure> (<mode>) (<size>)")
  sys.exit(0)
...
for test in sorted(os.listdir('data/'+problems)) :
  ...
  if os.path.isdir(testdir): 
    with open(testdir+'/ans') as ans:
      ...
      print("Testing "+test)
      if len(sys.argv) > 5:
        process = Popen(commands+['-d',testdir+'/dict','-m',str(mode),'-s',size,testdir+'/infile'], stdout=PIPE, stderr=PIPE)
      else:
      ...
      if (error and not error_printed):
        print("Expected output to start with Spellchecking: and it does not - nothing checked because of this")
      if (not error):
        print("Tests passed")
end = time.time()
