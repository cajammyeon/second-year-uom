...
public class bnb_kp extends KnapSack {
	...
	/* YOUR CODE GOES HERE */
	struc_sol solution_vector = new struc_sol();
	solution_vector.fixed = 0;
	frac_bound(solution_vector, solution_vector.fixed);

	// calculate current best
	// insert in priority queue
	int current_best = solution_vector.val;
	double upper_bound = solution_vector.bound;	
	insert(solution_vector);

	while (pqueue.length > 0 || upper_bound > current_best) {
				
		struc_sol parent_sol = removeMax();

		// set the true child
		struc_sol true_child = parent_sol.copy();
		true_child.fixed = parent_sol.fixed + 1;
		true_child.solution_vec.set(true_child.fixed, true);
		frac_bound(true_child, true_child.fixed);

		// set the false child
		struc_sol false_child = parent_sol.copy();
		false_child.fixed = parent_sol.fixed + 1;
		false_child.solution_vec.set(false_child.fixed, false);
		frac_bound(false_child, false_child.fixed);

		if (true_child.val != -1) {
			if (true_child.val > current_best) {
				current_best = true_child.val;
				copy_array(true_child.solution_vec, final_sol);
			}
			insert(true_child);
		} 

		if (false_child.val != -1) {
			if (false_child.val > current_best) {
				current_best = false_child.val;
				copy_array(false_child.solution_vec, final_sol);
			} 
			insert(false_child);
		}

		upper_bound = Math.floor((false_child.bound < true_child.bound) ? true_child.bound : false_child.bound);
		if ((int) upper_bound <= (int) current_best) {
			break;
		}
	}
	return final_sol;
