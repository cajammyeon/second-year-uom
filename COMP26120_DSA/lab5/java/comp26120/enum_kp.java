...
public class enum_kp extends KnapSack {
	...
	while (!(next_binary(solution, Nitems))) {
		...
		if (infeasible == 0) {
			...
			for (i = 1; i <= Nitems; i++) {
				value_calculated += (item_values.get(i) * (solution.get(i) ? 1 : 0));
			}

			// replace the current best value
			if (value_calculated > best_value) {
				best_value = value_calculated;
				best_solution.clear();
				best_solution.addAll(solution);
			}
		}
	}
		// unmap the value from hot encoding
		this.printer(best_value, best_solution);
    }

	private void printer(Integer best_value, ArrayList<Boolean> best_solution) {
		ArrayList<Integer> best_solution_unmapped = new ArrayList<Integer>();
		String pack_item = "";

		for (int k = 1; k <= Nitems; k++) {
			if (best_solution.get(k)) {
				best_solution_unmapped.add(k);
			}
		}
		for (Integer integer : best_solution_unmapped) {
			pack_item += (Integer.toString(integer) + " ");
		}
		System.out.println("Pack items: " + pack_item + "\nvalue=" + Integer.toString(best_value));
	}
