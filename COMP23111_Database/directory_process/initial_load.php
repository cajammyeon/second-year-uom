...
$stmt = $pdo->prepare("SELECT * FROM employee INNER JOIN position ON position.position = employee.position;");
$stmt->execute();
...
if (count($results) > 0) {
    foreach ($results as $row) {
        echo "<div class='employee_content' role='button' data-bs-toggle='modal' data-bs-target='#staticBackdrop". htmlspecialchars($row["unique_ID"]) . "'>
        ...
        echo '
        <div class="modal fade" id="staticBackdrop'. htmlspecialchars($row["unique_ID"]).'" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
